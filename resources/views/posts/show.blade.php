<x-layout>
        ...
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="flex items-center bg">
                    <h1 class="text-3xl font-bold tracking-tight text-white dark:text-gray-900">{{ $post['title'] }}</h1>
                    @auth    
            ...
            <span class="text-[17px] flex items-center gap-1 self-center ml-3">{{ $post->views }} 
    ...
    <div class=" flex flex-col justify-center gap-4 bg-white rounded-md">
        <img class="rounded-t-md" src="{{ asset('storage/'.$post->poster) }}" alt="">
    ...
    <div class="flex justify-between mt-5 border-gray-50">
        ...
        <p class="text-[16px] sm:text-[20px] mt-2 text-right">Created <span class="font-bold">{{ $post->created_at->diffForHumans() }}</span> by <a class="text-sky-500 hover:underline" href="/authors/{{ $post->user->slug }}">{{ $post->user->first_name }} {{ $post->user->last_name }}</a> </p>
    </div>
